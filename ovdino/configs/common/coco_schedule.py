...
def multi_steps_scheduler(
    ...
    total_steps = max(total_steps, _total_steps)
    decay_steps = [decay_epoch * steps_per_epoch for decay_epoch in decay_epochs]
    decay_steps.append(total_steps)
    values = [0.1**i for i in range(len(decay_epochs) + 1)]
    scheduler = L(MultiStepParamScheduler)(
        values=values,
